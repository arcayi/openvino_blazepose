# Blazepose tracking with ONNX Runtime

Running Google Mediapipe body pose tracking models on ONNX Runtime (**Updated with the models of mediapipe v0.8.10 2022/08**).
...
You need ONNX Runtime, OpenCV, open3d (for 3d visualization). installed on your computer and to clone/download this repository.

Note that the models were generated using converted from the original model 2022.8.
...
optional arguments:
  ...
  -d, --display         Output level to display
...
- To play back the detection result(with pose landmarks and connections) :

    ```python3 BlazeposeOpenvino.py -d1```

- To shutdown playback:

    ```python3 BlazeposeOpenvino.py -d0```
